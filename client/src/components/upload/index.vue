<template>
  <Upload v-model:file-list="fileList" name="file" :action="`${config?.baseUrl}/upload/imgs`" :headers="headers"
    enctype="multipart/form-data" :beforeUpload="handleBeforeUpload" :showUploadList="false" @change="handleChange">
    <Button>
      <div v-if="loading">
        <Spin />
      </div>
...
const handleBeforeUpload = async (file: any) => {
  ...
  if (props.isLarge) {
    try {
      ...
      Promise.all(allRequest).then(async res => {
        console.log('所有分片上传成功', fileName)
        const { url='', fileType='', fileName: _fileName } = await $fetch(`${config?.baseUrl}/upload/mergeFile`, {
          method: 'POST',
          query: {
            fileName: md5,
        ...
        emit('uploadSucess', { imgSrc: url, fileType, fileName: _fileName })
      }).finally(() => {
        loading.value = false
      })
